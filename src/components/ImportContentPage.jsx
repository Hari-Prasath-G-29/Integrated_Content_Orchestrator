...
//           <h1 className="page-title">Import Content for Adaptation</h1>
...
//                   <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
//                     <path d="M12 2l3.09 6.26L22 9.27l-5 4.87L18.18 22 12 18.9 5.82 22 7 14.14l-5-4.87 6.91-1.01L12 2z" stroke="#111" strokeWidth="1.6" />
//                   </svg>
...
//                     <div className="info-value">{selectedCard.subtitle || "—"}</div>
//                   </div>
//                 </div>
//               </div>
...
//                 <div className="section-header">
//                   <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
//                     <path d="M4 6h16M4 12h16M4 18h10" stroke="#111" strokeWidth="1.8" strokeLinecap="round" />
//                   </svg>
...
//   const source    = "veeva";
//   const text      = item.text_content || item.document_content || "No content available.";
...
export default function ImportContentPage() {
   ...
   * Fetches Veeva data on mount regardless of which tab is active.
   */
  useEffect(() => {
    if (veevaFetchedOnce) return;
...
function mapVeevaItem(item) {
  ...
  const title     = item.file_name ?? item.name ?? "(Untitled)";
  const type      = item.asset_type ?? item.contentType ?? "veeva_item";
  const statusRaw = item.status ?? item.status_v ?? "";
  const source    = "veeva";
  const text      = item.text_content || item.document_content || "No content available.";
  const description = item.description || "no description present";
...
function chipClass(status) {
  const s = (status || "").toLowerCase();
  if (s.includes("success")) return "success";
  if (s.includes("approved")) return "primary";
  return "muted";
}
